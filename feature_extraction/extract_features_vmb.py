...
class FeatureExtractor:
    ...
    def __init__(self, args=None):
        if args is None:
            ...
            args.setdefault("model_file", None)
            args.setdefault("config_file", None)
            args.setdefault("start_index", 0)
            args.setdefault("end_index", None)
            args.setdefault("background", False)
            args.setdefault("num_features", 100)
            args.setdefault("exclude_list", "./list")
            args.setdefault("feature_name", "fc6")
            ...
            info_list.append(
                {
                    ...
                    "img_w": im_infos[i]["width"],
                    "img_h": im_infos[i]["height"],
                }
            )
    ...
    def _save_feature(self, file_name, feature, info):
        ...
        file_base_name = file_base_name + ".pt"

        torch.save(
            info, os.path.join(self.args.output_folder, file_base_name) #feature.cpu().numpy()
        )
